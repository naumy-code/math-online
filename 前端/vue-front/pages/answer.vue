...
export default {
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      answer.getExamInfo(examCode).then(res => {  //通过examCode请求试卷详细信息
    ...
    commit() { //答案提交计算分数
      ...
      if(this.time != 0) {
        this.$confirm("考试结束时间未到,是否提前交卷","友情提示",{
          ...
          //提交成绩信息
          this.$axios({
            url: 'http://localhost:8222/examservice/Score/addScore',
            method: 'post',
            data: {
              studentId: this.loginInfo.id, // 用户编号
              ...
              this.$router.push({path:'/studentScore',query: {
                //topic1Answer: this.topic1Answer,  // 选择题信息
                examCode: this.examData.examCode, //考试编号
                score: finalScore, 
