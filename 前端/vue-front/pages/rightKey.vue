      ...
      <el-collapse v-model="activeName" >
        <el-collapse-item class="header" name="0">
          <template slot="title" class="stitle" >
            <div class="title">
              <span style="margin-left:8px;">{{examData.source}}</span><i class="header-icon el-icon-info"></i>
            ...
            <el-collapse-item>
              ...
              <div class="contenti">
                <ul class="question" v-for="(list, index) in topic[1]" :key="index">
                  ...
                  <li>A: {{list.answerA}} </li>
                  <li>B: {{list.answerB}} </li>
                  <li>C: {{list.answerC}} </li>
                  <li>D: {{list.answerD}} </li>
                  <li>正确答案: {{list.rightAnswer}} </li>
                  <!-- <li>你的答案: {{topic1Answer}} </li> -->
                  <li>试题解析：{{list.analysis}}</li>
                </ul>
              </div>
            </el-collapse-item>
            <el-collapse-item>
            ...
            <el-collapse-item>
              ...
              <div class="contenti">
                <ul class="question" v-for="(list, index) in topic[3]" :key="index">
                  <li>{{topicCount[0]+topicCount[1]+index+1}}. {{list.question}} {{list.rightAnswer}} {{list.score}}分</li>
                  <li>正确答案: {{`${list.answer==='T'?'正确':list.answer==='F'?'错误':''}`}} </li>
                  <li>试题解析：{{list.analysis}}</li>
                </ul>
              </div>
            </el-collapse-item>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      //let topic1Answer = this.$route.query.topic1Answer
      //console.log(topic1Answer)
      exam.getExamInfo(examCode).then(res => {  //通过examCode请求试卷详细信息
        res.data.data.examDate = res.data.data.examDate.substr(0,10)
        this.examData = { ...res.data.data}
        let paperId = this.examData.paperId
        //this.$axios(`http://localhost:8101/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
        this.$axios(`http://localhost:8222/examservice/paper/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          this.topic = {...res.data}
