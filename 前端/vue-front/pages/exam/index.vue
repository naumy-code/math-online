    ...
    <div class="wrapper">
      ...
      <ul class="paper" v-loading="loading">
        <li class="item" v-for="(item,index) in pagination.records" :key="index">
          ...
          <h4>
          <a :href="'/exam/'+item.examCode" :title="item.examCode" target="_blank">
              {{item.source}}
          </a>
          </h4>
...
export default {
  ...
  methods: {
    //获取当前所有考试信息
    getExamInfo() {
      this.$axios(`http://localhost:8101/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    //搜索试卷
    search() {
      this.$axios('http://localhost:8101/exams').then(res => {
