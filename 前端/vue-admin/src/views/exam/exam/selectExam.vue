...
<template>
  <div class="exam">
    ...
    >
      ...
      <el-table-column prop="institute" label="课程老师" width="120"></el-table-column>
      <el-table-column prop="major" label="课程名称" width="200"></el-table-column>
...
export default {
  ...
  methods: {
    edit(examCode) { //编辑试卷
      ...
      //this.$axios(`http://localhost:8101/exam/${examCode}`)
      exam.getExamInfo(examCode).then(res => { //根据试卷id请求后台
        if(res.code == 20000) {
          this.form = res.data
        }
      })
    ...
    submit() { //提交修改后的试卷信息
      ...
      exam.updateExamInfo(this.form).then(res => {
            this.$message({ //成功修改提示
              message: '更新成功',
              type: 'success'
            })
    ...
    deleteRecord(examCode) {
      this.$confirm("确定删除该记录吗,该操作不可逆！！！","删除提示",{
        ...
        // this.$axios({
        //   url: `http://localhost:8101/exam/${examCode}`,
        //   method: 'delete',
        // })
        exam.deleteExamId(examCode).then(res => {
          this.getExamInfo()
        })
      }).catch(() => {
    ...
    getExamInfo() { //分页查询所有试卷信息
      //this.$axios(`http://localhost:8101/exams/${this.pagination.current}/${this.pagination.size}`)
      exam
      .getExamListPage(this.pagination.current, this.pagination.size).then((response) => {
