      ...
      <section class="append">
        <ul>
          ...
          <li v-if="optionValue == '选择题'">
            ...
            <span>题目分数：</span>
            <el-input v-model="postChange.score" style="width:60px;"></el-input>
          </li>
          ...
          <li v-if="optionValue == '填空题'">
            ...
            <span>题目分数：</span>
            <el-input v-model="postFill.score" style="width:60px;"></el-input>
          </li>
          <li v-if="optionValue == '判断题'">
            <span>难度等级:</span>
            ...
            <span>题目分数：</span>
            <el-input v-model="postJudge.score" style="width:60px;"></el-input>
          </li>
          <li v-if="optionValue == '选择题'">
            <span>正确选项:</span>
        ...
        <div class="change" v-if="optionValue == '选择题'">
          <div class="title">
            <el-tag>题目:</el-tag><span>在下面的输入框中输入题目,形如15里面有（  ）个十和（  ）个一。</span>
            <el-input
              type="textarea"
              rows="4"
        ...
        <div class="change fill" v-if="optionValue == '填空题'">
          <div class="title">
            <el-tag>题目:</el-tag><span>输入题目,形如:1个十和2个一合起来是()。注意需要考生答题部分一定要用括号（英文半角）括起来。</span>
            <el-input
              type="textarea"
              rows="4"
...
<script>
import addExamQuestion from "@/api/exam/addExamQuestion";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    create() {
      this.$axios({
        ...
        data: {
          ...
          subject: '小学数学一年级' //题目数量太少？，指定为小学数学一年级出题
        }
      }).then(res => {
        console.log(res)
        let data = res.data
    ...
    getParams() {
      let subject = this.$route.params.source //获取试卷名称
      let paperId = this.$route.params.paperId //获取paperId  this.$route.params.id
    ...
    changeSubmit() { //选择题题库提交
      ...
      //   url: 'http://localhost:8101/MultiQuestion',
      //   method: 'post',
      //   data: {
      //     ...this.postChange          
      //   }
      ...
      addExamQuestion.addPaperMultiQuestion(this.postChange).then(res => { //添加成功显示提示
        ...
        //this.$axios(`http://localhost:8101/multiQuestionId`)
          addExamQuestion.getExamInfo().then(res => { //获取当前题目的questionId
          let questionId = res.data.questionId
          this.postPaper.questionId = questionId
          this.postPaper.questionType = 1

          // this.$axios({
          //   url: 'http://localhost:8101/paperManage',
          //   method: 'Post',
          //   data: {
          //     ...this.postPaper
          //   }
          // })
          addExamQuestion.addPaperMultiQuestionPaper(this.postPaper)
    ...
    fillSubmit() { //填空题提交
      ...
      // this.$axios({
      //   url: 'http://localhost:8101/fillQuestion',
      //   method: 'post',
      //   data: {
      //     ...this.postFill
      //   }
      // })
      addExamQuestion.addPaperfillQuestion(this.postFill).then(res => {
        // let status = res.data.code
        // if(status == 200) {
          this.$message({
            message: '已添加到题库',
        ...
        //this.$axios(`http://localhost:8101/fillQuestionId`)
        addExamQuestion.getExamfillQuestion().then(res => { //获取当前题目的questionId
          let questionId = res.data.questionId
          this.postPaper.questionId = questionId
    ...
    judgeSubmit() { //判断题提交
      ...
      // this.$axios({
      //   url: 'http://localhost:8101/judgeQuestion',
      //   method: 'post',
      //   data: {
      //     ...this.postJudge
      //   }
      // })
      addExamQuestion.addPaperjudgeQuestion(this.postJudge).then(res => {
        //let status = res.data.code
        //if(status == 200) {
          this.$message({
            message: '已添加到题库',
        ...
        // this.$axios(`http://localhost:8101/judgeQuestionId`)
        addExamQuestion.getExamjudgeQuestion().then(res => { //获取当前题目的questionId
          let questionId = res.data.questionId
          this.postPaper.questionId = questionId
          ...
          //   }
          // })
          addExamQuestion.addPapejudgeQuestionPaper(this.postPaper)
        })
      })
    }
  },
