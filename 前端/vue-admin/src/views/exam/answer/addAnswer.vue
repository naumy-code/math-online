    ...
    <el-table :data="list" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="add(scope.row.paperId,scope.row.source)" type="primary" size="small">增加习题</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import ExamQusetion from "@/api/exam/addExamQuestion";
export default {
  data() {
    return {
      list: null,
  ...
  methods: {
    getExamInfo() { //分页查询所有试卷信息
      ...
      ExamQusetion.getExamQusetionListPage(this.pagination.current, this.pagination.size).then(res => {
    ...
    add(paperId,source) { //增加题库
      //this.$router.push({path:'/exam/answer/addAnswerChildren/'+this.paperId&this.source})  ,query: {paperId: paperId,subject:source}
      this.$router.push({
          path:`/answer/edit/${paperId}/${source}`
      })
    }
  },
};
